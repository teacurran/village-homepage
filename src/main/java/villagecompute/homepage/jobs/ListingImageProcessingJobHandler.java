...
public class ListingImageProcessingJobHandler implements JobHandler {
    ...
    public void execute(Long jobId, Map<String, Object> payload) throws Exception {
        ...
        try {
            ...
            // 7. Export metrics
            meterRegistry.counter("marketplace.images.processed.total", "listing_id", listingId.toString()).increment();

            sample.stop(Timer.builder("marketplace.images.processing.duration").tag("status", "success")
                    .register(meterRegistry));

            LOG.infof("Successfully processed image variants for imageId=%s (3 variants created)", imageId);

        } catch (Exception e) {
            sample.stop(Timer.builder("marketplace.images.processing.duration").tag("status", "error")
                    .register(meterRegistry));

            meterRegistry
                    .counter("marketplace.images.processing.errors.total", "error_type", e.getClass().getSimpleName())
                    .increment();
     ...
     */
    private void createImageRecord(UUID listingId, StorageUploadResultType uploadResult, String variant,
            int displayOrder, String originalFilename) {
        ...
        LOG.debugf("Created %s variant record: storageKey=%s, displayOrder=%d", variant, uploadResult.objectKey(),
                displayOrder);
    }
}
