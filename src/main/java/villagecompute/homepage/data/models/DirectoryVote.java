 ...
 *
 * <p>Database mapping: directory_votes table</p>
 *
 * <p>Vote mechanics:
 * <ul>
 ...
 * </ul>
 *
 * <p>Voting is scoped to the site-category combination, following the principle
 * that a site might be excellent in one category but mediocre in another.</p>
 *
 * @see DirectorySiteCategory
 * @see DirectorySite
 */
@Entity
@Table(name = "directory_votes")
public class DirectoryVote extends PanacheEntityBase {
     ...
     *
     * @param siteCategoryId Site-category membership ID
    ...
    public static Optional<Short> getUserVote(UUID siteCategoryId, UUID userId) {
        Optional<DirectoryVote> vote = find("siteCategoryId = ?1 AND userId = ?2",
                siteCategoryId, userId).firstResultOptional();
    ...
    public static Optional<DirectoryVote> findByUserAndSiteCategory(UUID siteCategoryId, UUID userId) {
        return find("siteCategoryId = ?1 AND userId = ?2", siteCategoryId, userId)
                .firstResultOptional();
    }
     ...
     *
     * <p>This should be called after any vote is created, updated, or deleted
     * to keep the denormalized upvotes/downvotes/score values in sync.</p>
     */
