...
/**
 * Integration tests for Facebook OAuth 2.0 authentication flow.
 *
 * <p>
 * Tests cover:
 * <ul>
 ...
 */
@QuarkusTest
public class FacebookOAuthFlowTest extends WireMockTestBase {

    private static final Logger LOG = Logger.getLogger(FacebookOAuthFlowTest.class);
    ...
    protected void setUp() {
        ...
        System.setProperty("quarkus.rest-client.facebook-oauth.url", "http://localhost:" + wireMockServer.port());
    }

    /**
     * Tests successful Facebook OAuth login flow for a new user.
     *
     * <p>
     * Verifies:
     * <ul>
     ...
     * <li>Token exchange with Facebook
     * <li>User profile retrieval from Graph API
    ...
    public void testSuccessfulLogin() {
        ...
        OAuthUrlResponseType initResponse = oauthService.initiateFacebookLogin(TestConstants.TEST_SESSION_HASH,
        ...
        assertTrue(initResponse.authorizationUrl().contains("facebook.com"),
                "Auth URL should point to Facebook OAuth endpoint");

        // Verify state stored in database
        assertEntityExists(OAuthState.class,
                OAuthState.findByStateAndProvider(initResponse.state(), "facebook").get().id);

        // 2. Stub Facebook OAuth API responses
        stubFacebookTokenExchange();
        stubFacebookUserInfo();

        // 3. Handle OAuth callback
        User user = oauthService.handleFacebookCallback(TestConstants.TEST_OAUTH_CODE, initResponse.state(),
                TestConstants.TEST_OAUTH_REDIRECT_URI);
        ...
        assertEquals(TestConstants.TEST_FACEBOOK_EMAIL, user.email, "Email should match Facebook profile");
        assertEquals(TestConstants.OAUTH_PROVIDER_FACEBOOK, user.oauthProvider, "OAuth provider should be 'facebook'");
        assertEquals(TestConstants.TEST_FACEBOOK_USER_ID, user.oauthId, "OAuth ID should match Facebook user ID");
        ...
        assertTrue(OAuthState.findByStateAndProvider(initResponse.state(), "facebook").isEmpty(),
                "State token should be deleted after use to prevent replay attacks");
    }

    /**
     * Tests successful login for an existing Facebook OAuth user.
     *
     * <p>
    ...
    public void testExistingUserLogin() {
        // 1. Create existing user with same Facebook OAuth ID
        User existingUser = TestFixtures.createOAuthUser(TestConstants.TEST_FACEBOOK_EMAIL,
                TestConstants.OAUTH_PROVIDER_FACEBOOK, TestConstants.TEST_FACEBOOK_USER_ID);

        // 2. Initiate login flow
        OAuthUrlResponseType initResponse = oauthService.initiateFacebookLogin(TestConstants.TEST_SESSION_HASH,
                TestConstants.TEST_OAUTH_REDIRECT_URI);

        // 3. Stub Facebook OAuth API responses
        stubFacebookTokenExchange();
        stubFacebookUserInfo();

        // 4. Handle OAuth callback
        User returnedUser = oauthService.handleFacebookCallback(TestConstants.TEST_OAUTH_CODE, initResponse.state(),
                TestConstants.TEST_OAUTH_REDIRECT_URI);
    ...
    public void testInvalidState() {
        ...
        SecurityException exception = assertThrows(SecurityException.class, () -> {
            oauthService.handleFacebookCallback(TestConstants.TEST_OAUTH_CODE, "invalid-state-token",
                    TestConstants.TEST_OAUTH_REDIRECT_URI);
        ...
        assertTrue(User.findByEmail(TestConstants.TEST_FACEBOOK_EMAIL).isEmpty(),
    ...
    /**
     * Tests error handling when Facebook token exchange fails.
     *
     * <p>
    ...
    public void testTokenExchangeFailure() {
        ...
        // 2. Stub token endpoint to return error (invalid_grant)
        wireMockServer.stubFor(com.github.tomakehurst.wiremock.client.WireMock.post("/oauth/access_token")
        ...
        assertThrows(Exception.class, () -> {
            oauthService.handleFacebookCallback(TestConstants.TEST_OAUTH_CODE, initResponse.state(),
                    TestConstants.TEST_OAUTH_REDIRECT_URI);
    ...
    public void testAnonymousUpgrade() {
        ...
        OAuthUrlResponseType initResponse = oauthService.initiateFacebookLogin(anonUser.id.toString(),
                TestConstants.TEST_OAUTH_REDIRECT_URI);
        ...
        assertEquals(TestConstants.OAUTH_PROVIDER_FACEBOOK, authenticatedUser.oauthProvider);
        assertEquals(TestConstants.TEST_FACEBOOK_EMAIL, authenticatedUser.email);
