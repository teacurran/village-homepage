 ...
 *
 * Tests cover anonymous cookie generation, bootstrap guard, and login rate limiting. Extended to meet I2.T9 acceptance
 * criteria: "Coverage ≥80% for auth/preferences modules".
 *
...
class AuthIdentityServiceTest {
    ...
    void bootstrapCreatesValidSuperuser() {
        ...
        AuthIdentityService.BootstrapSession session = authIdentityService.createSuperuser("admin@example.com",
                "google", "google-admin-123");
    ...
    void assignRole_AddsRoleToUser() {
        ...
        assertTrue(authIdentityService.hasRole(user.id, "support"), "User should have assigned role");
    }
    ...
    void revokeRole_RemovesRoleFromUser() {
        ...
        UUID userId = QuarkusTransaction.requiringNew().call(() -> {
            User user = User.createAuthenticated("revoke-test@example.com", "google", "google-revoke-1", "Test User",
                    null);
            return user.id;
        });

        UUID adminId = QuarkusTransaction.requiringNew().call(() -> {
            User admin = User.createAuthenticated("revoke-admin@example.com", "google", "google-revoke-2", "Admin User",
                    null);
            return admin.id;
        });
    ...
    void getUserRole_ReturnsEmptyWhenNoRole() {
        ...
        assertTrue(authIdentityService.getUserRole(user.id).isEmpty(), "Should return empty when user has no role");
    }
}
